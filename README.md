...
- **proxy:** Optional upstream proxy for GitHub/GitHub Copilot traffic only. Set `enabled` to `true` and `url` to an HTTP, HTTPS, SOCKS, or SOCKS5 proxy URL, for example `"http://127.0.0.1:8080"` or `"socks5://127.0.0.1:1080"`. You can also use a shorthand string value such as `"socks5://127.0.0.1:1080"`. When configured, local client requests still hit this app directly, and this app connects to GitHub Copilot through the proxy.
