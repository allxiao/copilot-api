import type { Locale } from './index'

const en: Locale = {
  auth: {
    subtitle: 'Connect your GitHub Copilot',
    githubAuth: 'Sign in with GitHub',
    loading: 'Please wait…',
    manualToken: 'Enter token manually',
    deviceCode: 'Authorization code',
    deviceCodeUrl: 'Authorization URL',
    copy: 'Copy',
    copied: '✓ Copied',
    openAuthPage: 'Open authorization page',
    waitingAuth: 'Waiting for GitHub authorization…',
    back: '← Back',
    verifying: 'Verifying…',
    confirmAdd: 'Confirm',
    authFailed: 'Authorization failed, please try again',
    tokenInvalid: 'Invalid token, please try again',
    loginConsent: 'By signing in you authorize access to Copilot API',
  },
  dashboard: {
    invalidPort: 'Please enter a valid port (1–65535)',
    serverStopped: 'Server not running',
    configPort: 'Configure port and start with one click',
    port: 'Port',
    serverUnexpectedStop: 'Server stopped unexpectedly',
    starting: 'Starting…',
    startServer: '▶ Start server',
    tabDashboard: 'Dashboard',
    tabTokenUsage: 'Token usage',
    tabLogs: 'Logs',
    premiumUsed: 'Premium used',
    quotaReset: 'Quota reset',
    serviceAddress: 'Service address',
    authHeader: 'Auth header',
    copy: 'Copy',
    quotaUsage: 'Quota usage',
    refreshing: 'Refreshing…',
    refresh: '🔄 Refresh',
    tokenUsage: 'Token usage',
    tokenUsageAllModels: 'All models',
    tokenUsageCache: 'Cache',
    tokenUsageCacheRead: 'Cache read',
    tokenUsageCacheWrite: 'Cache write',
    tokenUsageEndpoint: 'Endpoint',
    tokenUsageEvents: 'Event details',
    tokenUsageInput: 'Input',
    tokenUsageModel: 'Model',
    tokenUsageModelBreakdown: 'Model breakdown',
    tokenUsageOutput: 'Output',
    tokenUsagePage: 'Page {{page}} / {{total}}',
    tokenUsagePeriodDay: 'Day',
    tokenUsagePeriodMonth: '30 days',
    tokenUsagePeriodWeek: '7 days',
    tokenUsageProvider: 'Provider',
    tokenUsageRequests: 'Requests',
    tokenUsageSession: 'Session',
    tokenUsageSource: 'Source',
    tokenUsageTime: 'Time',
    tokenUsageTotal: 'Total',
    tokenUsageTrend: 'Daily trend',
    tokenUsageTrace: 'Trace',
    tokenUsageUser: 'User',
    availableModels: 'Available models',
    modelsCount: '{{n}} models',
    next: 'Next',
    loading: 'Loading…',
    noModels: 'No models available',
    noTokenUsage: 'No token usage recorded',
    previous: 'Previous',
    serverLog: 'Server logs',
    clear: 'Clear',
    noLogs: 'No logs yet…',
  },
  header: {
    stop: '■ Stop',
    running: 'Running',
    notStarted: 'Not started',
    logout: 'Log out',
    settings: 'Settings',
    appSettings: 'App settings',
    advancedConfig: 'Advanced config',
  },
  tray: {
    showWindow: 'Show window',
    quit: 'Quit',
  },
  server: {
    tokenNotFound: 'Token not found',
    portInUse: 'Port {{port}} is already in use, please choose a different port',
    startFailed: 'Server process failed to start (exit code {{code}}); refer to the log panel below for details',
    startTimeout: 'Server startup timed out, port {{port}} may already be in use',
    processExit: 'Process exited with code {{code}}',
  },
  settings: {
    title: 'Settings',
    restartAppNote: 'Changes to OAuth App, API Home, and Enterprise URL take effect only after restarting the app.',
    restartAppPrompt: 'Saved. Restart the app for OAuth App, API Home, and Enterprise URL changes to take effect.',
    sectionGeneral: 'General',
    minimizeToTray: 'Minimize to tray',
    minimizeToTrayDesc: "Hide to system tray when closing, don't quit",
    sectionStartup: 'Startup',
    oauthApp: 'OAuth App',
    oauthAppDefault: 'Default GitHub',
    oauthAppDesc: 'When set to opencode, Electron will use the opencode OAuth flow on startup.',
    apiHome: 'API Home',
    apiHomeDesc: 'Optional custom directory for tokens and config.json.',
    enterpriseUrl: 'Enterprise URL',
    enterpriseUrlDesc: 'Optional value such as company.ghe.com for GitHub Enterprise.',
    verbose: 'Verbose logging',
    verboseDesc: 'Output more detailed debug information',
    showToken: 'Show token',
    showTokenDesc: 'Print GitHub/Copilot token values in logs',
    sectionLanguage: 'Language',
    langAuto: 'Follow system',
    langEn: 'English',
    langZh: '中文',
    cancel: 'Cancel',
    save: 'Save',
    saving: 'Saving…',
  },
  advancedConfig: {
    title: 'Advanced config',
    subtitle: 'Manage model mappings through the running service API so new POST /v1/messages requests pick up changes immediately.',
    back: 'Back to dashboard',
    proxyTitle: 'GitHub Copilot proxy',
    proxyDesc: 'Route GitHub and GitHub Copilot upstream traffic through an HTTP, HTTPS, SOCKS, or SOCKS5 proxy.',
    proxyEnabled: 'Use proxy',
    proxyUrl: 'Proxy URL',
    proxyProtocols: 'Supported protocols',
    proxyProtocolsDesc: 'Use http://, https://, socks://, or socks5://. Local clients still connect to this app directly.',
    proxyValidationRequired: 'Proxy URL is required when proxy is enabled.',
    modelMappingsTitle: 'Model mappings',
    modelMappingsDesc: 'Map the requested model name to another target model. Targets may be regular model IDs or provider/model aliases.',
    scopeLabel: 'Scope',
    scopeNote: 'Only applies to POST /v1/messages. Matching is exact and the app will not trim or normalize the model name for you.',
    restartNote: 'Changes are applied to the running service immediately after a successful save.',
    configPath: 'Config path',
    addMapping: 'Add mapping',
    sourceModel: 'Requested model',
    targetModel: 'Target model',
    remove: 'Remove',
    emptyTitle: 'No mappings yet',
    emptyDescription: 'Add a rule to remap an incoming model name before the Messages API resolves the actual upstream model.',
    saveHelp: 'Proxy settings can be saved before the service starts. Model mappings require the running service API.',
    saved: 'Saved through the service API. New upstream requests will use the updated settings immediately.',
    proxySaved: 'Proxy settings saved. Start the service to connect upstream through the configured proxy.',
    loadFailed: 'Failed to load advanced config',
    saveFailed: 'Failed to save advanced config',
    serverRequired: 'Start the local service before loading or saving advanced config through the API.',
    validationIncomplete: 'Each mapping row must include both a requested model and a target model.',
    validationDuplicate: 'Duplicate requested model: {{model}}',
  },
}

export default en
