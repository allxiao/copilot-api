...
const zh: Locale = {
  ...
  advancedConfig: {
    ...
    back: '返回看板',
    proxyTitle: 'GitHub Copilot 代理',
    proxyDesc: '将 GitHub 和 GitHub Copilot 的上游流量通过 HTTP、HTTPS、SOCKS 或 SOCKS5 代理转发。',
    proxyEnabled: '启用代理',
    proxyUrl: '代理 URL',
    proxyProtocols: '支持协议',
    proxyProtocolsDesc: '支持 http://、https://、socks:// 或 socks5://。本地客户端仍然直接连接本应用。',
    proxyValidationRequired: '启用代理时必须填写代理 URL。',
    ...
    saveHelp: '代理设置可在服务启动前保存；模型映射需要通过运行中的服务接口保存。',
    saved: '已通过服务接口保存。后续新的上游请求会立即使用最新设置。',
    proxySaved: '代理设置已保存。启动服务后，上游连接会使用已配置的代理。',
    loadFailed: '加载高级配置失败',
    saveFailed: '保存高级配置失败',
